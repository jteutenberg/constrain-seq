...
func main() {
	...
	aligner := align.NewCostedAligner(*threads, alphabet, []align.Constraint{constraint, align.NewNoHomopolymerConstraint(alphabet), align.NewMaxLengthConstraint(*maxLength)}, align.NewRLECost(5), align.NewFixedCost(5))

	p := pipeline.NewPipeline()
	p.Append(util.NewLineReader(*inputFile))
	p.Append(seq.NewFastAReader(1))
	p.Append(rle.NewRunLengthEncoder(1))
	p.Append(aligner)
